#!/usr/bin/env python3
"""
SPT Safe Driving Token Video Script Tracking Dashboard
=====================================================

A comprehensive analytics and progress monitoring system for the SPT Token Rewards Video Script Generation project.
...
Features:
- Real-time progress tracking (0/55 stories completed - curated pool ready)
...
Author: Claude Code
Project: SPT Safe Driving Token Video Script Generation System
"""
...
class TrackingDashboard:
    """
    Analytics and tracking dashboard for SPT Safe Driving Token Video Script Generation project.
    ...
    def get_overview_stats(self) -> Dict:
        """
        Get high-level project statistics including failed stories and URL correction stats.
        
        Returns:
            Dict: Statistics including total stories, completion counts, failed count, and URL correction data
        ...
        failed = sum(1 for s in self.stories if s.get('status') == 'failed')
        correction_failed = sum(1 for s in self.stories if s.get('status') == 'correction_failed')
        ...
        total_processable = len(self.stories) - failed - correction_failed
        completion_rate = (completed / total_processable * 100) if total_processable > 0 else 0
        ...
        return {
            ...
            'processable': total_processable,
            'url_correction': {
                'attempted': correction_attempted,
                'succeeded': correction_succeeded,
                'failed': correction_attempted - correction_succeeded,
    ...
    def get_failed_stories(self, limit: int = 10) -> List[Dict]:
        """Get stories that failed processing"""
        failed_stories = [s for s in self.stories if s.get('status') == 'failed']
        # Sort by failed_date descending (most recent first)
        sorted_stories = sorted(
            failed_stories, 
            key=lambda x: x.get('failed_date', ''), 
            reverse=True
        )
    ...
    def print_dashboard(self):
        ...
        print("=" * 80)
        print("SPT VIDEO SCRIPT TRACKING DASHBOARD")
        print("=" * 80)
        ...
        if stats['correction_failed'] > 0:
            print(f"🔧 Correction Failed: {stats['correction_failed']}")
        if stats['failed'] > 0 or stats['correction_failed'] > 0:
            print(f"📈 Processable: {stats['processable']} (excluding failed)")
            
        # URL correction stats
        if stats['url_correction']['attempted'] > 0:
            print(f"\n🔧 URL CORRECTION STATS")
            print(f"Attempted: {stats['url_correction']['attempted']}")
            print(f"✅ Succeeded: {stats['url_correction']['succeeded']} ({stats['url_correction']['success_rate']})")
            print(f"❌ Failed: {stats['url_correction']['failed']}")
        ...
        failed = self.get_failed_stories(5)
        if failed:
            print("\n❌ FAILED STORIES")
            for story in failed:
                date = story.get('failed_date', 'Unknown')
                reason = story.get('failure_reason', 'Unknown reason')[:40]
                print(f"ID {story['id']:3} | {date} | {reason} | {story['title'][:35]}...")
